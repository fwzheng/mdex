...
# MDeX v2.1.0 (macOS · Windows · Linux · 오프라인 대응 · Tauri v2)
...
- **AI 보조 편집**: 텍스트를 선택하고 `⌘/Ctrl + J`(또는 툴바 버튼)를 누르면 AI가 이를 다듬거나, 확장하거나, 어조를 바꿔 줍니다; 온라인 서비스(OpenAI / DeepSeek / Anthropic / GLM / Gemini / Kimi 등)와 완전한 오프라인 사용을 위한 Ollama 같은 로컬 모델 모두에서 작동합니다; 결과는 원본을 교체하기 전에 미리 보여주며, API 키는 로컬에만 저장됩니다.
