...
(function () {
  ...
  const I18N = {
    zh: {
      ...
      aiRewrite: "AI 改写", aiSettings: "AI 设置", aiSettingsTitle: "AI 改写设置",
      ...
      aiPh: "输入指令（改写/提问/生成）… Enter 发送、Esc 关闭", aiSend: "发送", aiApply: "应用到文档", aiDiscard: "关闭",
      aiInsert: "在光标处插入", aiEditedNote: "✓ 已改写（见编辑区）", aiChatZone: "对话", aiEditZone: "编辑区",
      aiToEdit: "→ 用作编辑区内容", aiMovedToEdit: "已设为编辑区内容，可点「应用到文档」",
      aiEditUndo: "回退到上一个编辑状态", aiEditRedo: "前进到下一个编辑状态", aiViewRender: "渲染", aiViewSrc: "源码",
      aiModeRewrite: "改写", aiModeAsk: "提问", aiAskPh: "针对选中的片段提问…（Enter 发送 / Esc 取消）", aiCopy: "复制", aiCopied: "已复制到剪贴板", aiCopyFail: "复制失败",
    ...
    en: {
      ...
      aiRewrite: "AI Rewrite", aiSettings: "AI Settings", aiSettingsTitle: "AI Rewrite Settings",
      ...
      aiPh: "Type an instruction (rewrite/ask/generate)… Enter to send, Esc to close", aiSend: "Send", aiApply: "Apply", aiDiscard: "Close",
      aiInsert: "Insert at cursor", aiEditedNote: "✓ Rewritten (see edit zone)", aiChatZone: "Chat", aiEditZone: "Edit zone",
      ...
      aiModeRewrite: "Rewrite", aiModeAsk: "Ask", aiAskPh: "Ask about the selected text… (Enter to send / Esc to cancel)", aiCopy: "Copy", aiCopied: "Copied to clipboard", aiCopyFail: "Copy failed",
  ...
  */
  ...
  function buildHelp(s) {
    ...
    return [
      ...
      "<p>" + s.pIntro.replace("{ver}", appVersion || "2.0.0") + "</p>",
  ...
  const HELP_STRINGS = {
    zh: {
      hIntro: "简介",
      pIntro: "MDeX v{ver} — 一款主打隐私保护与AI辅助的多语言 Markdown 阅读与编辑器。</p><p><strong>隐私至上，数据安全</strong><br>所有文件均在本地处理，默认不联网、不上传、无云端同步，确保数据绝对安全。采用原子化保存机制，无惧崩溃或断电导致的数据丢失。</p><p><strong>AI 辅助，离线无忧</strong><br>自 v2.0 起可选接入 AI 辅助改写。AI 功能同样支持全程离线：接入本地模型（如 Ollama）时完全不联网；仅当用户主动配置并触发在线 AI 服务（OpenAI / Anthropic 等）时，才会发起网络请求。</p><p>🔗 源代码：github.com/fwzheng/mdex<br>📥 下载站点：spinss.cn",
      pPron: "读作 “em-dex”（/ˌemˈdɛks/）—— 字母 M 接 “dex”，共两个音节。",
      hFeatures: "核心功能",
      features: [
        { b: "AI 辅助改写（可选）", t: "：选中文字或不选（光标处插入），按 <code>⌘J</code> / 工具栏 ✨ / 右键唤出 AI 浮层。浮层分<strong>编辑区</strong>（改写结果与原文 diff 高亮，可 ↶回退 / ↷前进、应用到文档或在光标处插入）与<strong>对话区</strong>（多轮对话）；改写类指令实时进编辑区、问答类进对话区。支持 OpenAI 兼容接口（含本地 Ollama，可全程离线）与 Anthropic 双协议，密钥仅存本机、不进会话备份。默认关闭，需在「编辑 → AI 设置」配置后启用。" },
      ...
      shortcuts: sc(["新建","打开","保存","另存为","关闭标签页","关闭窗口","查找","替换","加粗","斜体","行内代码","链接","缩进 2 空格","矩形（列）选取","扩展列选取","取消列选取","AI 改写"]),
      hMd: "Markdown 语法速查",
      md: [
    ...
    en: {
      hIntro: "Overview",
      pIntro: "MDeX v{ver} — a multilingual Markdown reader & editor focused on privacy and AI assistance.</p><p><strong>Privacy first, data safe</strong><br>All files are processed locally — no network, no uploads, no cloud sync by default, keeping your data fully secure. Atomic (crash-proof) saves — no data loss on crash or power outage.</p><p><strong>AI-assisted, offline-ready</strong><br>Since v2.0, optional AI-assisted rewriting. AI can also stay fully offline: a local model (e.g., Ollama) needs no internet; only an online AI service (OpenAI / Anthropic, etc.) makes a request, and only when you configure and trigger it.</p><p>🔗 Source: github.com/fwzheng/mdex<br>📥 Download: spinss.cn",
      pPron: "Pronounced “em-dex” (/ˌemˈdɛks/) — the letter M followed by “dex”, two syllables.",
      hFeatures: "Features",
      features: [
        { b: "AI-assisted rewrite (optional)", t: ": select text or place the cursor, then press <code>⌘J</code> / toolbar ✨ / right-click to open the AI pane. It splits into an <strong>Edit zone</strong> (rewrite diff-highlighted vs the original; ↶ undo / ↷ redo, apply to doc or insert at cursor) and a <strong>Chat zone</strong> (multi-turn). Rewrite instructions stream into the Edit zone, Q&A into the Chat zone. Supports OpenAI-compatible endpoints (incl. a local Ollama, which stays fully offline) and Anthropic; the key stays on your machine and is never included in session backups. Off by default — enable it via Edit → AI Settings." },
      ...
      shortcuts: sc(["New","Open","Save","Save As","Close Tab","Close Window","Find","Replace","Bold","Italic","Inline code","Link","Indent 2 spaces","Rectangular (column) select","Extend column select","Cancel column select","AI rewrite"]),
    ...
    fr: {
      ...
      shortcuts: sc(["Nouveau","Ouvrir","Enregistrer","Enregistrer sous","Fermer l'onglet","Fermer la fenêtre","Rechercher","Remplacer","Gras","Italique","Code en ligne","Lien","Indenter 2 espaces","Sélection rectangulaire (colonne)","Étendre la sélection","Annuler la sélection","Réécriture IA"]),
    ...
    de: {
      ...
      shortcuts: sc(["Neu","Öffnen","Speichern","Speichern unter","Tab schließen","Fenster schließen","Suchen","Ersetzen","Fett","Kursiv","Inline-Code","Link","2 Leerzeichen einrücken","Rechteckige (Spalten-) Auswahl","Auswahl erweitern","Auswahl abbrechen","AI-Umschreiben"]),
    ...
    ru: {
      ...
      shortcuts: sc(["Создать","Открыть","Сохранить","Сохранить как","Закрыть вкладку","Закрыть окно","Найти","Заменить","Полужирный","Курсив","Код","Ссылка","Отступ 2 пробела","Прямоугольное (колоночное) выделение","Расширить выделение","Отменить выделение","Перезапись AI"]),
    ...
    it: {
      hIntro: "Panoramica",
      pIntro: "MDeX v{ver} — un lettore ed editor Markdown multilingue focalizzato su privacy e assistenza AI.</p><p><strong>Privacy prima di tutto, dati al sicuro</strong><br>Tutti i file vengono elaborati localmente — nessuna rete, nessun caricamento, nessuna sincronizzazione cloud per impostazione predefinita, mantenendo i tuoi dati completamente al sicuro. Salvataggi atomici (a prova di crash) — nessuna perdita di dati in caso di crash o interruzione di corrente.</p><p><strong>Assistito da AI, pronto offline</strong><br>Dalla v2.0, riscrittura assistita da AI opzionale. Anche l'AI può restare completamente offline: un modello locale (es. Ollama) non richiede internet; solo un servizio AI online (OpenAI / Anthropic, ecc.) effettua una richiesta, e solo quando lo configuri e lo attivi.</p><p>🔗 Sorgente: github.com/fwzheng/mdex<br>📥 Download: spinss.cn",
      pPron: "Si pronuncia «em-dex» (/ˌemˈdɛks/) — la lettera M seguita da «dex», due sillabe.",
      hFeatures: "Funzionalità",
      features: [
        { b: "Riscrittura assistita da AI (opzionale)", t: ": seleziona del testo o posiziona il cursore, quindi premi ⌘J / ✨ nella barra / clic destro per aprire il pannello AI. Si divide in una <strong>zona di modifica</strong> (riscrittura evidenziata come diff rispetto all'originale; ↶ annulla / ↷ ripristina, applica al documento o inserisci al cursore) e una <strong>zona di chat</strong> (multi-turno). Le istruzioni di riscrittura fluiscono nella zona di modifica, domande e risposte nella zona di chat. Supporta endpoint compatibili con OpenAI (incluso un Ollama locale, che resta completamente offline) e Anthropic; la chiave resta sul tuo computer e non viene mai inclusa nei backup di sessione. Disattivata per impostazione predefinita — attivala tramite Modifica → Impostazioni AI." },
      ...
      shortcuts: sc(["Nuovo","Apri","Salva","Salva con nome","Chiudi scheda","Chiudi finestra","Trova","Sostituisci","Grassetto","Corsivo","Codice in linea","Collegamento","Rientro 2 spazi","Selezione rettangolare (colonna)","Estendi selezione","Annulla selezione","Riscrittura AI"]),
    ...
    ja: {
      hIntro: "概要",
      pIntro: "MDeX v{ver} — プライバシーとAIアシスタンスに重点を置いた多言語Markdownリーダー＆エディター。</p><p><strong>プライバシー優先、データは安全</strong><br>すべてのファイルはローカルで処理されます — ネットワーク不要、アップロード不要、デフォルトでクラウド同期なしで、データを完全に保護します。アトミック（クラッシュ耐性）保存 — クラッシュや停電時にもデータ損失はありません。</p><p><strong>AIアシスト機能、オフライン対応</strong><br>v2.0以降、オプションでAIアシストによる書き換え機能を利用できます。AIも完全にオフラインで利用可能です：ローカルモデル（例：Ollama）はインターネット不要で、オンラインAIサービス（OpenAI / Anthropic など）のみが、設定してトリガーした場合にのみリクエストを送信します。</p><p>🔗 ソース：github.com/fwzheng/mdex<br>📥 ダウンロード：spinss.cn",
      pPron: "「em-dex」と発音します（/ˌemˈdɛks/）—— 文字 M に続けて「dex」の2音節です。",
      hFeatures: "機能",
      features: [
        { b: "AI アシストリライト（オプション）", t: "：テキストを選択するかカーソルを置き、<code>⌘J</code> / ツールバー ✨ / 右クリックで AI パネルを開きます。パネルは<strong>編集ゾーン</strong>（リライト結果を原文と diff ハイライト；↶ 元に戻す / ↷ やり直し、ドキュメントに適用またはカーソル位置に挿入）と<strong>チャットゾーン</strong>（複数ターン）に分かれます。リライト指示は編集ゾーンへ、Q&A はチャットゾーンへストリーミングされます。OpenAI 互換エンドポイント（完全オフラインで動作するローカル Ollama を含む）と Anthropic に対応；キーはお使いの端末にのみ保存され、セッションバックアップには含まれません。デフォルトはオフ —「編集 → AI 設定」から有効にしてください。" },
      ...
      shortcuts: sc(["新規","開く","保存","名前を付けて保存","タブを閉じる","ウィンドウを閉じる","検索","置換","太字","斜体","インラインコード","リンク","2スペース字下げ","矩形（列）選択","選択を拡張","選択をキャンセル","AI 書き換え"]),
      hMd: "Markdown チートシート",
      md: [
    ...
    ko: {
      hIntro: "개요",
      pIntro: "MDeX v{ver} — 개인정보 보호와 AI 지원에 중점을 둔 다국어 Markdown 리더 및 에디터입니다.</p><p><strong>개인정보 보호 우선, 안전한 데이터</strong><br>모든 파일은 로컬에서 처리됩니다 — 기본적으로 네트워크 연결, 업로드, 클라우드 동기화가 없어 데이터가 완벽하게 안전하게 보호됩니다. 원자적(크래시 방지) 저장 — 크래시나 정전 시에도 데이터 손실이 없습니다.</p><p><strong>AI 지원, 오프라인 대응</strong><br>v2.0부터 선택적 AI 지원 다시쓰기를 제공합니다. AI는 완전히 오프라인으로도 유지할 수 있습니다: 로컬 모델(예: Ollama)은 인터넷이 필요 없으며, 온라인 AI 서비스(OpenAI / Anthropic 등)만이 요청을 보내고, 그것도 사용자가 구성하고 실행할 때만 해당합니다.</p><p>🔗 소스: github.com/fwzheng/mdex<br>📥 다운로드: spinss.cn",
      pPron: "“em-dex”로 발음합니다 (/ˌemˈdɛks/) — 문자 M 뒤에 “dex”가 오는 두 음절입니다.",
      hFeatures: "기능",
      features: [
        { b: "AI 보조 다시쓰기 (선택)", t: ": 텍스트를 선택하거나 커서를 둔 뒤 ⌘J / 도구모음 ✨ / 우클릭으로 AI 패널을 엽니다. 패널은 <strong>편집 영역</strong>(원본과 diff 하이라이트된 다시쓰기 결과; ↶ 실행 취소 / ↷ 다시 실행, 문서에 적용 또는 커서에 삽입)과 <strong>대화 영역</strong>(다중 턴)으로 나뉩니다. 다시쓰기 지시는 편집 영역에, 질의응답은 대화 영역에 스트리밍됩니다. OpenAI 호환 엔드포인트(완전히 오프라인으로 유지되는 로컬 Ollama 포함)와 Anthropic 지원; 키는 본인 기기에만 저장되며 세션 백업에 포함되지 않습니다. 기본 꺼짐 — 「편집 → AI 설정」에서 활성화." },
      ...
      shortcuts: sc(["새로 만들기","열기","저장","다른 이름으로 저장","탭 닫기","창 닫기","찾기","바꾸기","굵게","기울임","인라인 코드","링크","2칸 들여쓰기","사각형(열) 선택","선택 확장","선택 취소","AI 다시 쓰기"]),
      hMd: "Markdown 요약",
      md: [
    ...
    es: {
      hIntro: "Descripción",
      pIntro: "MDeX v{ver} — un lector y editor de Markdown multilingüe centrado en la privacidad y la asistencia con AI.</p><p><strong>Privacidad primero, datos seguros</strong><br>Todos los archivos se procesan localmente — sin red, sin cargas, sin sincronización en la nube por defecto, manteniendo sus datos totalmente seguros. Guardado atómico (a prueba de fallos) — sin pérdida de datos ante un cuelgue o corte de energía.</p><p><strong>Asistencia con AI, listo sin conexión</strong><br>Desde v2.0, reescritura opcional con asistencia de AI. La AI también puede permanecer totalmente sin conexión: un modelo local (p. ej., Ollama) no necesita internet; solo un servicio de AI en línea (OpenAI / Anthropic, etc.) realiza una petición, y únicamente cuando lo configuras y lo activas.</p><p>🔗 Fuente: github.com/fwzheng/mdex<br>📥 Descarga: spinss.cn",
      pPron: "Se pronuncia «em-dex» (/ˌemˈdɛks/) — la letra M seguida de «dex», dos sílabas.",
      hFeatures: "Funciones",
      features: [
        { b: "Reescritura con asistencia de AI (opcional)", t: ": selecciona texto o coloca el cursor y pulsa ⌘J / el botón ✨ de la barra / clic derecho para abrir el panel de AI. Se divide en una <strong>zona de edición</strong> (reescritura con diff resaltado frente al original; ↶ deshacer / ↷ rehacer, aplicar al documento o insertar en el cursor) y una <strong>zona de chat</strong> (multi-turno). Las instrucciones de reescritura se transmiten a la zona de edición, y las preguntas y respuestas a la zona de chat. Admite endpoints compatibles con OpenAI (incluido un Ollama local, que funciona totalmente offline) y Anthropic; la clave permanece en tu equipo y nunca se incluye en las copias de seguridad de sesión. Desactivado por defecto — actívalo mediante Edición → Ajustes de AI." },
      ...
      shortcuts: sc(["Nuevo","Abrir","Guardar","Guardar como","Cerrar pestaña","Cerrar ventana","Buscar","Reemplazar","Negrita","Cursiva","Código en línea","Enlace","Sangrar 2 espacios","Selección rectangular (columna)","Extender selección","Cancelar selección","Reescritura IA"]),
    ...
    pt: {
      ...
      shortcuts: sc(["Novo","Abrir","Salvar","Salvar como","Fechar guia","Fechar janela","Buscar","Substituir","Negrito","Itálico","Código em linha","Link","Recuar 2 espaços","Seleção retangular (colona)","Estender seleção","Cancelar seleção","Reescrita IA"]),
    ...
    ar: {
      hIntro: "نظرة عامة",
      pIntro: "MDeX v{ver} — قارئ ومحرر Markdown متعدد اللغات يركّز على الخصوصية والمساعدة عبر AI.</p><p><strong>الخصوصية أولاً، والبيانات آمنة</strong><br>تتم معالجة جميع الملفات محلياً — بدون شبكة، وبدون رفع، وبدون مزامنة سحابية افتراضياً، مما يحافظ على أمان بياناتك بالكامل. الحفظ الذري (المقاوم للأعطال) — لا فقدان للبيانات عند الانهيار أو انقطاع التيار.</p><p><strong>مساعدة AI وجاهز للعمل دون اتصال</strong><br>منذ v2.0، إعادة الكتابة بمساعدة AI اختيارية. ويمكن لـ AI أيضاً البقاء دون اتصال تماماً: النموذج المحلي (مثل Ollama) لا يحتاج إلى إنترنت؛ فقط خدمة AI عبر الإنترنت (OpenAI / Anthropic، إلخ) تُجري طلباً، وذلك فقط عندما تقوم بتهيئتها وتشغيلها.</p><p>🔗 المصدر: github.com/fwzheng/mdex<br>📥 التحميل: spinss.cn",
      pPron: "يُنطق «em-dex» (/ˌemˈdɛks/) — الحرف M متبوعًا بـ «dex»، مقطعان لفظيان.",
      hFeatures: "الميزات",
      features: [
        { b: "إعادة الكتابة بمساعدة AI (اختياري)", t: ": حدّد نصًا أو ضع المؤشر، ثم اضغط ⌘J / زر الشريط ✨ / النقر بالزر الأيمن لفتح لوحة AI. تنقسم إلى <strong>منطقة تحرير</strong> (تُعرض إعادة الكتابة مع تمييز الفروق مقابل الأصل؛ ↶ تراجع / ↷ إعادة، التطبيق على المستند أو الإدراج عند المؤشر) و<strong>منطقة محادثة</strong> (متعددة الأدوار). تتدفق تعليمات إعادة الكتابة إلى منطقة التحرير، والأسئلة والأجوبة إلى منطقة المحادثة. يدعم نقاط نهاية متوافقة مع OpenAI (بما فيها Ollama محلي يبقى دون اتصال بالكامل) وAnthropic؛ يظل المفتاح على جهازك ولا يُضمَّن أبدًا في نسخ الجلسة الاحتياطية. معطَّل افتراضيًا — فعّله عبر تحرير ← إعدادات AI." },
      ...
      shortcuts: sc(["جديد","فتح","حفظ","حفظ باسم","إغلاق التبويب","إغلاق النافذة","بحث","استبدال","عريض","مائل","كود سطري","رابط","إزاحة مسافتين","تحديد مستطيل (عمود)","توسيع التحديد","إلغاء التحديد","إعادة الكتابة AI"]),
      hMd: "مرجع Markdown",
      md: [
    ...
    hi: {
      ...
      shortcuts: sc(["नया","खोलें","सहेजें","नाम से सहेजें","टैब बंद","विंडो बंद","खोजें","बदलें","बोल्ड","तिरछा","इनलाइन कोड","लिंक","2 रिक्त जगह इंडेंट","आयताकार (कॉलम) चयन","कॉलम चयन बढ़ाएँ","चयन रद्द","AI पुनर्लेखन"]),
      hMd: "Markdown संदर्भ",
      md: [
    ...
    pa: {
      hIntro: "ਜਾਣ-ਪਛਾਣ",
      pIntro: "MDeX v{ver} — ਇੱਕ ਬਹੁ-ਭਾਸ਼ੀ Markdown ਰੀਡਰ ਅਤੇ ਐਡੀਟਰ ਜੋ ਪ੍ਰਾਈਵੇਸੀ ਅਤੇ AI ਸਹਾਇਤਾ 'ਤੇ ਕੇਂਦਰਿਤ ਹੈ।</p><p><strong>ਪਹਿਲਾਂ ਪ੍ਰਾਈਵੇਸੀ, ਡਾਟਾ ਸੁਰੱਖਿਅਤ</strong><br>ਸਾਰੀਆਂ ਫਾਈਲਾਂ ਲੋਕਲ ਤੌਰ 'ਤੇ ਪ੍ਰੋਸੈਸ ਕੀਤੀਆਂ ਜਾਂਦੀਆਂ ਹਨ — ਕੋਈ ਨੈੱਟਵਰਕ ਨਹੀਂ, ਕੋਈ ਅੱਪਲੋਡ ਨਹੀਂ, ਡਿਫ਼ਾਲਟ ਰੂਪ ਵਿੱਚ ਕੋਈ ਕਲਾਊਡ ਸਿੰਕ ਨਹੀਂ, ਤੁਹਾਡੇ ਡਾਟੇ ਨੂੰ ਪੂਰੀ ਤਰ੍ਹਾਂ ਸੁਰੱਖਿਅਤ ਰੱਖਦਾ ਹੈ। ਐਟਮਿਕ (ਕ੍ਰੈਸ਼-ਪਰੂਫ) ਸੇਵ — ਕ੍ਰੈਸ਼ ਜਾਂ ਪਾਵਰ ਆਊਟੇਜ ਹੋਣ 'ਤੇ ਕੋਈ ਡਾਟਾ ਨੁਕਸਾਨ ਨਹੀਂ।</p><p><strong>AI-ਸਹਾਇਤਾ, ਆਫਲਾਈਨ-ਤਿਆਰ</strong><br>v2.0 ਤੋਂ, ਵਿਕਲਪਿਕ AI-ਸਹਾਇਤਾ ਰੀਰਾਈਟਿੰਗ। AI ਪੂਰੀ ਤਰ੍ਹਾਂ ਆਫਲਾਈਨ ਵੀ ਰਹਿ ਸਕਦਾ ਹੈ: ਇੱਕ ਲੋਕਲ ਮਾਡਲ (ਜਿਵੇਂ Ollama) ਨੂੰ ਇੰਟਰਨੈੱਟ ਦੀ ਲੋੜ ਨਹੀਂ; ਸਿਰਫ਼ ਇੱਕ ਆਨਲਾਈਨ AI ਸੇਵਾ (OpenAI / Anthropic, ਆਦਿ) ਬੇਨਤੀ ਕਰਦੀ ਹੈ, ਅਤੇ ਉਹ ਵੀ ਸਿਰਫ਼ ਜਦੋਂ ਤੁਸੀਂ ਇਸਨੂੰ ਸੰਰਚਿਤ ਅਤੇ ਚਲਾਇਆ।</p><p>🔗 ਸਰੋਤ: github.com/fwzheng/mdex<br>📥 ਡਾਊਨਲੋਡ: spinss.cn",
      pPron: "ਉਚਾਰਨ “em-dex” (/ˌemˈdɛks/) — ਅੱਖਰ M ਮਗਰੋਂ “dex”, ਦੋ ਅੱਖਰ।",
      hFeatures: "ਖਾਸੀਅਤਾਂ",
      features: [
        { b: "AI ਮਦਦ ਨਾਲ ਮੁੜ-ਲਿਖਣਾ (ਚੋਣਵਾਂ)", t: ": ਟੈਕਸਟ ਚੁਣੋ ਜਾਂ ਕਰਸਰ ਰੱਖੋ, ਫਿਰ AI ਪੈਨ ਖੋਲ੍ਹਣ ਲਈ <code>⌘J</code> / ਟੂਲਬਾਰ ✨ / ਸੱਜਾ-ਕਲਿੱਕ ਦਬਾਓ। ਇਹ ਇੱਕ <strong>ਸੰਪਾਦਨ ਜ਼ੋਨ</strong> (ਮੁੜ-ਲਿਖਤ ਮੂਲ ਦੇ ਮੁਕਾਬਲੇ diff-ਹਾਈਲਾਈਟ ਕੀਤੀ; ↶ ਵਾਪਸ / ↷ ਮੁੜ ਕਰੋ, ਦਸਤਾਵੇਜ਼ ਉੱਤੇ ਲਾਗੂ ਜਾਂ ਕਰਸਰ ਉੱਤੇ ਪਾਓ) ਅਤੇ ਇੱਕ <strong>ਗੱਲਬਾਤ ਜ਼ੋਨ</strong> (ਬਹੁ-ਪਾਸੀ) ਵਿੱਚ ਵੰਡਦਾ ਹੈ। ਮੁੜ-ਲਿਖਣ ਹਿਦਾਇਤਾਂ ਸੰਪਾਦਨ ਜ਼ੋਨ ਵਿੱਚ ਵਹਿੰਦੀਆਂ ਹਨ, ਸਵਾਲ-ਜਵਾਬ ਗੱਲਬਾਤ ਜ਼ੋਨ ਵਿੱਚ। OpenAI-ਅਨੁਕੂਲ ਐਂਡਪੌਇੰਟ (ਜਿਵੇਂ ਇੱਕ ਸਥਾਨਕ Ollama, ਜੋ ਪੂਰੀ ਤਰ੍ਹਾਂ ਆਫਲਾਈਨ ਰਹਿੰਦਾ ਹੈ) ਅਤੇ Anthropic ਨੂੰ ਸਮਰਥਨ ਦਿੰਦਾ ਹੈ; ਕੁੰਜੀ ਤੁਹਾਡੀ ਮਸ਼ੀਨ ਉੱਤੇ ਰਹਿੰਦੀ ਹੈ ਅਤੇ ਕਦੇ ਸੈਸ਼ਨ ਬੈਕਅੱਪ ਵਿੱਚ ਸ਼ਾਮਲ ਨਹੀਂ ਹੁੰਦੀ। ਮੂਲ ਰੂਪ ਵਿੱਚ ਬੰਦ — ਇਸਨੂੰ Edit → AI Settings ਰਾਹੀਂ ਚਾਲੂ ਕਰੋ।" },
      ...
      shortcuts: sc(["ਨਵਾਂ","ਖੋਲ੍ਹੋ","ਸੰਭਾਲੋ","ਨਾਮ ਨਾਲ ਸੰਭਾਲੋ","ਟੈਬ ਬੰਦ","ਵਿੰਡੋ ਬੰਦ","ਖੋਜੋ","ਬਦਲੋ","ਬੋਲਡ","ਤਿਰਛਾ","ਇਨਲਾਈਨ ਕੋਡ","ਲਿੰਕ","2 ਸਪੇਸ ਇੰਡੈਂਟ","ਆਇਤਾਕਾਰ (ਕਾਲਮ) ਚੋਣ","ਕਾਲਮ ਚੋਣ ਵਧਾਓ","ਚੋਣ ਰੱਦ","AI ਮੁੜ-ਲਿਖਣਾ"]),
      hMd: "Markdown ਹਵਾਲਾ",
      md: [
    ...
    vi: {
      ...
      shortcuts: sc(["Mới","Mở","Lưu","Lưu thành","Đóng thẻ","Đóng cửa sổ","Tìm","Thay","Đậm","Nghiêng","Mã nội dòng","Liên kết","Thụt 2 dấu cách","Chọn cột chữ nhật","Mở rộng chọn cột","Hủy chọn","Viết lại AI"]),
    ...
    id: {
      hIntro: "Ikhtisar",
      pIntro: "MDeX v{ver} — pembaca & editor Markdown multibahasa yang berfokus pada privasi dan bantuan AI.</p><p><strong>Privasi utama, data aman</strong><br>Semua file diproses secara lokal — tanpa jaringan, tanpa unggahan, tanpa sinkronisasi cloud secara default, menjaga data Anda tetap sepenuhnya aman. Penyimpanan atomik (tahan crash) — tidak ada kehilangan data saat crash atau mati listrik.</p><p><strong>Berbantuan AI, siap offline</strong><br>Sejak v2.0, penulisan ulang berbantuan AI opsional. AI juga dapat tetap sepenuhnya offline: model lokal (mis., Ollama) tidak memerlukan internet; hanya layanan AI online (OpenAI / Anthropic, dll.) yang membuat permintaan, dan hanya saat Anda mengonfigurasi dan memicunya.</p><p>🔗 Sumber: github.com/fwzheng/mdex<br>📥 Unduh: spinss.cn",
      pPron: "Dibaca “em-dex” (/ˌemˈdɛks/) — huruf M diikuti “dex”, dua suku kata.",
      hFeatures: "Fitur",
      features: [
        { b: "Penulisan ulang berbantuan AI (opsional)", t: ": pilih teks atau tempatkan kursor, lalu tekan ⌘J / tombol bilah alat ✨ / klik kanan untuk membuka panel AI. Terbagi menjadi <strong>zona Edit</strong> (penulisan ulang dengan penyorotan diff terhadap aslinya; ↶ urungkan / ↷ ulangi, terapkan ke dokumen atau sisipkan pada kursor) dan <strong>zona Obrolan</strong> (multi-putaran). Instruksi penulisan ulang mengalir ke zona Edit, tanya-jawab ke zona Obrolan. Mendukung endpoint kompatibel OpenAI (termasuk Ollama lokal, yang tetap sepenuhnya luring) dan Anthropic; kunci tetap di mesin Anda dan tidak pernah disertakan dalam cadangan sesi. Nonaktif secara bawaan — aktifkan melalui Edit → Pengaturan AI." },
      ...
      shortcuts: sc(["Baru","Buka","Simpan","Simpan Sebagai","Tutup Tab","Tutup Jendela","Cari","Ganti","Tebal","Miring","Kode sebaris","Tautan","Indentasi 2 spasi","Pilihan kolom (persegi)","Perluas pilihan kolom","Batal pilihan","Penulisan ulang AI"]),
    ...
    ur: {
      ...
      shortcuts: sc(["نیا","کھولیں","محفوظ","اس نام سے محفوظ","ٹیب بند","ونڈو بند","تلاش","تبدیل","موٹا","ترچھا","ان لائن کوڈ","ربط","2 اسپیس انڈینٹ","مستطیل (کالم) انتخاب","کالم انتخاب بڑھائیں","انتخاب منسوخ","AI دوبارہ تحریر"]),
      hMd: "Markdown حوالہ",
      md: [
    ...
    mn: {
      ...
      shortcuts: sc(["Шинэ","Нээх","Хадгалах","Өөр нэрээр хадгалах","Таб хаах","Цонх хаах","Хайх","Солих","Тод","Налуу","Шугаман код","Холбоос","2 зайгаар нүүх","Тэгш өнцөгт (баган) сонголт","Баган сонголтыг өргөтгөх","Сонголтыг цуцлах","AI дахин бичих"]),
      hMd: "Markdown лавлах",
      md: [
