...
/// 工作区（Phase 1b）：选文件夹（原生对话框）。用户取消返回 None。
#[tauri::command]
...
/// 工作区：枚举单层目录（文件树懒加载——展开才枚举子目录，大 vault 不全量扫）。
/// 只返回 .md/.markdown/.html 与子目录（隐藏目录 .xxx、node_modules、.git 跳过）。
/// 条目：{name, path, isDir}，目录在前、按名排序（自然序，中文按 Unicode 码点）。
#[tauri::command]
fn list_dir(dir: String) -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in entries.flatten() {
        ...
        if is_dir {
            // 常见非内容目录跳过（对齐 Obsidian 习惯）
            if name == "node_modules" || name == "__pycache__" || name == "target" {
                continue;
            }
            dirs.push((name, path.to_string_lossy().into_owned()));
        } else {
            let lower = name.to_lowercase();
            if lower.ends_with(".md") || lower.ends_with(".markdown") || lower.ends_with(".html") || lower.ends_with(".htm") {
    ...
    for (name, path) in dirs {
        out.push(serde_json::json!({ "name": name, "path": path, "isDir": true }));
    }
    for (name, path) in files {
        out.push(serde_json::json!({ "name": name, "path": path, "isDir": false }));
    }
    Ok(out)
}

/// 工作区：全 vault wikilink 索引（反向链接用）。异步命令跑 Tokio worker，不阻塞 UI。
/// 递归收集所有 .md/.markdown 的 [[目标]]（行级正则，与前端 scanWikilinks 语法一致：
/// 排除 ![[嵌入]] / \[[ / [[[ / [[x]](url)；# 开头 heading 链接不计入跨文件索引），
/// 返回 { files: [{path, mtime}], links: [{src, target, alias}] }——links 供反链查询，
/// files 的 mtime 供前端做增量重扫（只重读变化的文件）。
#[tauri::command]
async fn scan_wiki_index(root: String) -> Result<serde_json::Value, String> {
    let root_p = std::path::PathBuf::from(&root);
    if !root_p.is_dir() {
        return Err(format!("不是目录: {root}"));
    }
    // 递归收集 md 文件（隐藏目录/node_modules 等跳过，与 list_dir 规则一致）
    ...
    let md_files = tokio::task::spawn_blocking(move || {
        let mut files = Vec::new();
        collect_md(&root_p, &mut files);
        files
    })
    .await
    .map_err(|e| e.to_string())?;

    // 行级正则抽 [[目标]]（与前端 scanWikilinks 同款语境排斥；(?!\() 需 look-ahead，用手写扫描替代）
    fn extract_links(text: &str) -> Vec<(String, Option<String>)> {
        ...
        while i + 1 < n {
            if bytes[i] == '[' && bytes[i + 1] == '[' {
                ...
                if let Some(end) = close {
                    ...
                    // 排斥 [[x]](url)：紧跟 '('
                    let after = end + 2;
                    if after < n && bytes[after] == '(' {
                        i = after;
                        continue;
                    }
                    // 拆 |别名
                    let (target, alias) = match inner.find('|') {
                        Some(p) => (inner[..p].trim().to_string(), Some(inner[p + 1..].trim().to_string())),
                        None => (inner.trim().to_string(), None),
                    };
                    if !target.is_empty() && !target.starts_with('#') {
                        links.push((target, alias.filter(|a| !a.is_empty())));
                    }
                    i = after;
                } else {
                    i += 2;
    ...
    for f in &md_files {
        let mtime = f
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let text = match read_text_lossless(f) {
            Ok(t) => t,
            Err(_) => continue, // 读不了的文件跳过（不阻断整库索引）
        };
        let src = match f.strip_prefix(&root_path) {
            Ok(rel) => rel.to_string_lossy().replace('\\', "/"),
            Err(_) => f.to_string_lossy().into_owned(),
        };
        for (target, alias) in extract_links(&text) {
            links.push(serde_json::json!({ "src": src, "target": target, "alias": alias }));
        }
        files.push(serde_json::json!({ "path": src, "mtime": mtime }));
    }
...
fn labels(lang: &str) -> Labels {
    match lang {
        "en" => Labels {
            ...
            cite_example: "Citation Example…", mermaid_example: "Mermaid Examples…", wiki_example: "Wikilink Example…", close_file: "Close Tab", save: "Save", save_as: "Save As…",
            ...
            toggle_sidebar: "Toggle Sidebar", close_workspace: "Close Workspace",
        ...
        "fr" => Labels {
            ...
            cite_example: "Exemple de citation…", mermaid_example: "Exemples Mermaid…", wiki_example: "Exemple Wikilink…", close_file: "Fermer l'onglet", save: "Enregistrer", save_as: "Enregistrer sous…",
            ...
            toggle_sidebar: "Basculer la barre latérale", close_workspace: "Fermer l’espace de travail",
        ...
        "de" => Labels {
            ...
            cite_example: "Zitierbeispiel…", mermaid_example: "Mermaid-Beispiele…", wiki_example: "Wikilink-Beispiel…", close_file: "Tab schließen", save: "Speichern", save_as: "Speichern unter…",
            ...
            toggle_sidebar: "Seitenleiste umschalten", close_workspace: "Arbeitsbereich schließen",
        ...
        "ru" => Labels {
            ...
            cite_example: "Пример цитирования…", mermaid_example: "Примеры Mermaid…", wiki_example: "Пример Wikilink…", close_file: "Закрыть вкладку", save: "Сохранить", save_as: "Сохранить как…",
            ...
            toggle_sidebar: "Боковая панель", close_workspace: "Закрыть рабочую область",
        ...
        "it" => Labels {
            ...
            cite_example: "Esempio di citazione…", mermaid_example: "Esempi Mermaid…", wiki_example: "Esempio Wikilink…", close_file: "Chiudi scheda", save: "Salva", save_as: "Salva con nome…",
            ...
            toggle_sidebar: "Barra laterale", close_workspace: "Chiudi area di lavoro",
        ...
        "ja" => Labels {
            ...
            cite_example: "文献引用の例…", mermaid_example: "Mermaid の例…", wiki_example: "Wikilink の例…", close_file: "タブを閉じる", save: "保存", save_as: "名前を付けて保存…",
            ...
            toggle_sidebar: "サイドバー切替", close_workspace: "ワークスペースを閉じる",
        ...
        "ko" => Labels {
            ...
            load_bib: "문헌고 불러오기…",
            clear_bib: "문헌고 해제",
            cite_example: "인용 예시…", mermaid_example: "Mermaid 예시…", wiki_example: "Wikilink 예시…", close_file: "탭 닫기", save: "저장", save_as: "다른 이름으로 저장…",
            ...
            toggle_sidebar: "사이드바 전환", close_workspace: "작업 공간 닫기",
        ...
        "es" => Labels {
            ...
            cite_example: "Ejemplo de citación…", mermaid_example: "Ejemplos de Mermaid…", wiki_example: "Ejemplo de Wikilink…", close_file: "Cerrar pestaña", save: "Guardar", save_as: "Guardar como…",
            ...
            toggle_sidebar: "Alternar barra lateral", close_workspace: "Cerrar espacio de trabajo",
        ...
        "pt" => Labels {
            ...
            cite_example: "Exemplo de citação…", mermaid_example: "Exemplos do Mermaid…", wiki_example: "Exemplo de Wikilink…", close_file: "Fechar guia", save: "Salvar", save_as: "Salvar como…",
            ...
            toggle_sidebar: "Alternar barra lateral", close_workspace: "Fechar espaço de trabalho",
        ...
        "ar" => Labels {
            ...
            cite_example: "مثال على الاستشهاد…", mermaid_example: "أمثلة Mermaid…", wiki_example: "مثال Wikilink…", close_file: "إغلاق التبويب", save: "حفظ", save_as: "حفظ باسم…",
            ...
            toggle_sidebar: "تبديل الشريط الجانبي", close_workspace: "إغلاق مساحة العمل",
        ...
        "hi" => Labels {
            ...
            cite_example: "उद्धरण उदाहरण…", mermaid_example: "Mermaid उदाहरण…", wiki_example: "Wikilink उदाहरण…", close_file: "टैब बंद करें", save: "सहेजें", save_as: "नाम से सहेजें…",
            ...
            toggle_sidebar: "साइडबार टॉगल", close_workspace: "वर्कस्पेस बंद करें",
        ...
        "pa" => Labels {
            ...
            open_folder: "ਫ਼ੋਲਡਰ ਖੋਲ੍ਹੋ…",
            load_bib: "ਗ੍ਰੰਥ-ਸੂਚੀ ਲੋਡ ਕਰੋ…",
            clear_bib: "ਗ੍ਰੰਥ-ਸੂਚੀ ਅਨਲੋਡ",
            cite_example: "ਹਵਾਲਾ ਉਦਾਹਰਣ…", mermaid_example: "Mermaid ਉਦਾਹਰਣ…", wiki_example: "Wikilink ਉਦਾਹਰਣ…", close_file: "ਟੈਬ ਬੰਦ", save: "ਸੰਭਾਲੋ", save_as: "ਨਾਮ ਨਾਲ ਸੰਭਾਲੋ…",
            ...
            toggle_sidebar: "ਸਾਈਡਬਾਰ ਬਦਲੋ", close_workspace: "ਵਰਕਸਪੇਸ ਬੰਦ ਕਰੋ",
        ...
        "vi" => Labels {
            ...
            cite_example: "Ví dụ trích dẫn…", mermaid_example: "Ví dụ Mermaid…", wiki_example: "Ví dụ Wikilink…", close_file: "Đóng thẻ", save: "Lưu", save_as: "Lưu thành…",
            ...
            toggle_sidebar: "Chuyển đổi thanh bên", close_workspace: "Đóng không gian làm việc",
        ...
        "id" => Labels {
            ...
            cite_example: "Contoh Kutipan…", mermaid_example: "Contoh Mermaid…", wiki_example: "Contoh Wikilink…", close_file: "Tutup Tab", save: "Simpan", save_as: "Simpan Sebagai…",
            ...
            toggle_sidebar: "Alihkan bilah samping", close_workspace: "Tutup ruang kerja",
        ...
        "ur" => Labels {
            ...
            cite_example: "اقتباس مثال…", mermaid_example: "Mermaid مثالیں…", wiki_example: "Wikilink مثال…", close_file: "ٹیب بند کریں", save: "محفوظ کریں", save_as: "اس نام سے محفوظ…",
            ...
            toggle_sidebar: "سائیڈبار ٹوگل", close_workspace: "ورک اسپیس بند کریں",
        ...
        "mn" => Labels {
            ...
            new: "Шинэ", open: "Нээх…",
            open_folder: "Фолдер нээх…",
            load_bib: "Номын сан ачаалах…",
            clear_bib: "Номын сан ачаалахгүй болгох",
            cite_example: "Эшлэлийн жишээ…", mermaid_example: "Mermaid жишээ…", wiki_example: "Wikilink жишээ…", close_file: "Таб хаах", save: "Хадгалах", save_as: "Өөр нэрээр хадгалах…",
            ...
            toggle_sidebar: "Хажуугийн самбар шилжүүлэх", close_workspace: "Ажлын талбар хаах",
        ...
        _ => Labels {
            ...
            load_bib: "加载文献库…",
            clear_bib: "卸载文献库",
            cite_example: "文献引用示例…", mermaid_example: "Mermaid 图示例…", wiki_example: "Wikilink 示例…", close_file: "关闭标签页", save: "保存", save_as: "另存为…",
            ...
            toggle_sidebar: "切换侧栏", close_workspace: "关闭工作区",
...
fn build_menu(app: &tauri::AppHandle, lang: &str) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let m_openfolder = MenuItem::with_id(app, "open-folder", l.open_folder, true, None::<&str>)?;
    ...
    let m_wikiex = MenuItem::with_id(app, "wiki-example", l.wiki_example, true, None::<&str>)?;
    ...
    // ===== 视图 =====
    // 视图菜单：切换侧栏（带加速键——隐藏后 × 不可见，唯一键盘恢复途径）+ 关闭工作区
    let m_sidebar = MenuItem::with_id(app, "toggle-sidebar", l.toggle_sidebar, true, Some("CmdOrCtrl+Shift+B"))?;
    let m_closews = MenuItem::with_id(app, "close-workspace", l.close_workspace, true, None::<&str>)?;
