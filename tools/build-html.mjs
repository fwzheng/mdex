...
async function build() {
  ...
  if (!shell.includes("<!--VENDOR:css-->") || !shell.includes("<!--VENDOR:js-->")) {
    console.error("✗ app-shell.html 缺少 <!--VENDOR:css--> 或 <!--VENDOR:js--> 标记");
    process.exit(1);
  }
  // 必须用函数替换：若用字符串，vendor 代码里的 $& $` $' $n 会被当成 replace 的特殊模式
  // （例如 katex 的 "$&" 会被替换成匹配串 "<!--VENDOR:js-->"，导致脚本损坏、公式无法渲染）。
  shell = shell.replace("<!--VENDOR:css-->", () => vendorCss).replace("<!--VENDOR:js-->", () => vendorJs);
