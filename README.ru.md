...
Многоязычное приложение для чтения и редактирования Markdown с акцентом на **конфиденциальность и поддержку ИИ**. Каждый файл обрабатывается локально — **по умолчанию без сети, без загрузок на сервер, без облачной синхронизации, без рекламы, без телеметрии**; атомарное (устойчивое к сбоям) сохранение исключает потерю данных при сбое или отключении питания. Начиная с v2.0, MDeX также предлагает опциональное редактирование текста с помощью ИИ — и при этом может оставаться полностью автономным: локальной модели (например, Ollama) интернет не требуется; сетевой запрос выполняет только онлайн-сервис ИИ (OpenAI / DeepSeek / Anthropic / GLM / Gemini / Kimi и т. д.), и только когда вы его настроите и запустите.
...
- **Редактирование с помощью ИИ**: выделите текст и нажмите `⌘/Ctrl + J` (или кнопку на панели инструментов), чтобы ИИ доработал, расширил или изменил его тон; работает с онлайн-сервисами (OpenAI / DeepSeek / Anthropic / GLM / Gemini / Kimi и т. д.) и с локальной моделью, такой как Ollama, для полностью автономного использования; результат показывается в предпросмотре перед заменой оригинала, а API-ключ хранится только локально.
...
| `⌘/Ctrl + J` | Редактирование с помощью ИИ |
