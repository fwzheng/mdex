...
- **AI アシスト編集**：テキストを選択して `⌘/Ctrl + J`（またはツールバーボタン）を押すと、AI が推敲・拡張・トーン調整を行います；オンラインサービス（OpenAI / DeepSeek / Anthropic / GLM / Gemini / Kimi など）および Ollama のようなローカルモデルに対応し、完全オフラインでも利用可能；結果は元のテキストを置き換える前にプレビュー表示され、API キーはローカルにのみ保存されます。
